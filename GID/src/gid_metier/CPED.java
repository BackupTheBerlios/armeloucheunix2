	   ...
	   /*try
		{
	        conn = ds.getConnection();
			s = conn.createStatement();
			res = s.executeQuery("SELECT cped.*,comptabilite.id as comptabilite_id, solde , acteur_id FROM cped LEFT JOIN comptabilite ON (acteur_id = cped.id) WHERE id='" + id + "'");
			if (res.next())
			{
 ...
 */
    ...
    public void authentifie(String login, String password) throws NamingException
	{
	    ...
	    /*try
		{
	        conn = ds.getConnection();
			s = conn.createStatement();
			res = s.executeQuery("SELECT cped.*,comptabilite.id as comptabilite_id, solde , acteur_id FROM cped LEFT JOIN comptabilite ON (acteur_id = cped.id) WHERE login='" + login + "' AND mdp='" + password + "'");
			if (res.next())
			{
			    this.setId(res.getInt("id"));
				this.setLogin(res.getString("login"));
